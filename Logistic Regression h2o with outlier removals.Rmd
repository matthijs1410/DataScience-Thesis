...
```
```{r}
smote_trn_h2o2 <- as.h2o(smote_trn2)
smote_dev_h2o2 <- as.h2o(smote_dev2)
...
default_lgr2 <- h2o.glm(
  y = y,
  training_frame = smote_trn_h2o2,
  validation_frame = smote_dev_h2o2,
  family = "binomial",
  nfolds = folds,
  seed = 69)
```
...
roc_curve_default_lgr2 <- default_roc_data_lgr2 %>%
  ...
  theme_light() +
  labs(
    title = "Logistic Regression ROC Curve Mahalanobis removal",
    x = "False Positive Rate (1 - Specificity)",
    y = "True Positive Rate (Sensitivity)"
  )
